...
if __name__ == '__main__':
	...
	parser.add_argument('addr', help="Payload address")
	...
	localization_data = sock.makefile()
	...
	for line in localization_data:
		...
		if 'ping' in data.keys():
			...
			pings_kml = simplekml.Kml()
			for ping in pings:
				pings_kml.newpoint(name=str(ping.seq), coords=[(ping.lat, ping.lon)])
			pings_kml.save('test.kml')
