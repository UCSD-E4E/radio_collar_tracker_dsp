...
void init_buffers(){
	...
	fft_plan 	= fftw_plan_dft_1d(pul_num_sam, fft_in, fft_out, FFTW_FORWARD, FFTW_MEASURE);
	fft_abs  	= (double*)malloc(pul_num_sam*sizeof(double)); 
	gain  		= (char*)malloc(num_fra_p_file*sizeof(char)); 
...
void load_files(){ 
	...
	for (int i = 0; i < num_col; i++){
		...
		fft_bin[i]    	= (int)ceil(pul_num_sam/2+((double)pul_num_sam)*((double)dem_goal_f[i])/f_samp+1);
	}
	...
	max_jump            = (int)ceil(((double)(MAX_SLICING_FACTOR*pul_num_sam))/1000); 
	min_jump            = (int)ceil(((double)(MIN_SLICING_FACTOR*pul_num_sam))/1000);
...
void run_fft(){	
	...
	for (int i = 0; i < pul_num_sam/2; i++){
		fft_abs[i] 					= cabs(fft_out[i+pul_num_sam/2])/pul_num_sam; 
...
void analysis(){	
	for (int i = 0; i < num_files; i++){
	    ...
	    // Raw file loading
		sprintf(aux_path,"%s%s%06d_%06d",raw_data_path,RAW_FILE_PREFIX,curr_run,i+1);
		fileStream = fopen(aux_path, "r");
		...
		for (int j = 0; j < num_fra_p_file; j++){
	        ...
	        for (int k = 0; k < 4; k++){
		        aux1 = ((int)data[(j+1)*file_frame_size-EXTRA_DATA_SIZE+0+k])&0xFF; 
	        ...
	        // Gain Parsing
			gain[j] = data[(j+1)*file_frame_size-1];
		}
		...
		for (int j = 0; j < num_fra_p_file; j++){
	        ...
	        cur_gain    = valid_gain_values[(int)gain[j]];
