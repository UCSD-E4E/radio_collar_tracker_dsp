...
static void print_fft()
{
	int i;
	for(i = 0; i < fft_size; i++)
	{
		(void)printf("%f,", log_pwr_fft[i]);
	}
	(void)printf("\n");
}

static void print_boop()
{
    int threshold = -50;
	  static unsigned int time = 0;
	  if(log_pwr_fft[fft_size/2+3] >= threshold)
	  {
		    (void) printf("%d boop\n", time);
#ifdef TEENSY_CONNECTED   
        // Request data
        write(serPort, &mode, 1);
...
int main(int argc, char *argv[])
{
	...
	(void)printf("fft_size=%d\n", fft_size);
	(void)printf("samp_rate=%d\n", samp_rate);

    setup_rtlsdr();

    tid = g_timeout_add(PRINT_RATE, timeout_cb, window);
