...
#define BUTTON_BROWSE_LABEL 			"Browse"
...
#define BROWSE_TITLE					"File Browser"
...
public:	
	...
	Elements(){
		...
		window->begin();
			...
			file_chooser 			= new Fl_File_Chooser ("/","*",0,BROWSE_TITLE);

			aux_y += ELEMENT_H+MARGIN+2*MARGIN;
			aux_x = aux_w-3*BUTTON_W-MARGIN;
			aux_w = BUTTON_W;
			aux_h = ELEMENT_H;
			button_browse 			= new Fl_Button		(aux_x,aux_y,aux_w,aux_h,BUTTON_BROWSE_LABEL); 
		...
		button_browse 			->labelfont(FL_HELVETICA);
		...
		button_browse 			->labelsize(TEXT_S);
...
void button_browse_cb(Fl_Widget *w, void *v);
...
int main(int argc, char **argv) {
	...
	elements->button_browse 			->callback(button_browse_cb,elements);
...
void browse(void *v){
	Elements* e = (Elements*)v;
	e->file_chooser->value(e->cfg_path->value());
	e->file_chooser->show();
	while(e->file_chooser->shown())
		Fl::wait();
	if (e->file_chooser->value() != NULL)
		e->cfg_path->value(e->file_chooser->value());
}
